    ...
    $: {
        ...
        history = history.sort((a, b) => {
            ...
            if (sortBy === "attempts") {
                ...
                return (
                    calcIncorrectAttempts(first) - calcIncorrectAttempts(second)
                );
...
>
    <div class="flex gap-8">
        ...
        >
            <span slot="prepend:label" class="mr-1 text-sm opacity-50"
                >Sorted By:
            </span>
        </Select>
        <Checkbox
    ...
    <div class="min-w-[40rem]">
                ...
                <div slot="item" let:index let:style {style} class="p-1">
                    <Card class="w-full h-full">
                        ...
                        <span class="ml-4 mb-2">
                            <span class="text-[0.8em] opacity-70"
                                >Last attempt</span
                            >
                            <span class="font-medium text-sm opacity-90"
                                >{formatDateTime(
                                    history[index].lastAttempt
                                )}</span
                            >
                        </span>
                        <div
                            role="status"
                            aria-live="polite"
                            class="grid grid-cols-3 justify-items-center mt-2"
                        >
                            <div class="flex flex-col-reverse items-center">
                                ...
                                <span
                                    ...
                                    class="text-[2em] font-bold text-sky-200"
                                    >{humanizeNumber(
                                        history[index].stats.attempts
                                    )}</span
                                >
                            </div>
                            <div class="flex flex-col-reverse items-center">
                                <span
                                    class="text-sm opacity-50 uppercase tracking-wide font-medium"
                                    >Correct Attempts</span
                                >
                                <span
                                    aria-label="Correct attempts"
                                    class="text-[2em] font-bold text-emerald-200"
                                    >{humanizeNumber(
                                        history[index].stats.correct
                                    )}</span
                                >
                            </div>
                            <div class="flex flex-col-reverse items-center">
                                <span
                                    class="text-sm opacity-50 uppercase tracking-wide font-medium"
                                    >Incorrect Attempts</span
                                >
                                <span
                                    aria-label="Incorrect attempts"
                                    class="text-[2em] font-bold text-rose-200"
                                    >{humanizeNumber(
                                        calcIncorrectAttempts(history[index])
                                    )}</span
                                >
                            </div>
                        </div>
        ...
        {:else}
            <div style:height={`${parentHeight - 100}px`} />
        {/if}
    </div>
</div>
